...
export const getAllProducts = async (req, res, next) => {
  ...
  try{
  const data = await getAllProductsRepo(req.query);
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    const reviewUser = reviews.map(review => new ObjectId(review.user).equals(new ObjectId(req.user._id)));

    console.log(reviewUser);

    if(reviewUser.includes(false)){
      return next(new ErrorHandler(403, "You dont have access to delete this review"));
    }
